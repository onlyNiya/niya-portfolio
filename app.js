const projects = [
  {
    id: "rednote",
    title: "小红书自媒体运营",
    titleEn: "REDNOTE CONTENT OPERATION",
    category: "CONTENT STRATEGY · SOCIAL MEDIA",
    year: "2024.08—至今",
    summary:
      "自 2024 年 8 月起独立运营数码 3C 与摄影内容账号，负责账号定位、用户研究、选题、脚本、出镜与拍摄、剪辑发布、评论区运营、数据复盘及商务沟通，已实现商业变现。",
    role: "独立完成策略、创作、运营与商务全流程",
    output: "57 篇可见笔记 / 数码 3C 商业内容",
    status: "独立运营",
    link: "https://xhslink.com/m/4iGDtDW0oiV",
    linkLabel: "访问小红书主页 / Visit profile",
    metrics: [
      ["10.4W", "粉丝 / Followers"],
      ["77.7W", "获赞与收藏 / Engagement"],
      ["57", "可见笔记 / Posts"],
    ],
    palette: {
      bg: "#741b29",
      ink: "#fff7f5",
      visual:
        "radial-gradient(circle at 0% 100%, #090808 0%, #180a0d 16%, rgba(31, 9, 14, .9) 30%, rgba(31, 9, 14, 0) 64%), linear-gradient(135deg, #8c2636 0%, #731a29 58%, #54101d 100%)",
      image:
        "radial-gradient(circle at 0% 100%, #090808 0%, #210a0f 30%, transparent 66%), linear-gradient(135deg, #862334, #54101d)",
    },
    gallery: [],
  },
  {
  ...
  },
  {
    ...
    gallery: ["cover", "editorial", "editorial", "research"],
  },
  {
    id: "visual-design",
    title: "平面与版式设计",
...
function renderGallery(project) {
  ...
  if (project.id === "campus-media") return createCampusGallery();
...
function handleHashRoute() {
  ...
  const caseMatch = window.location.hash.match(/^#project\/rednote\/case\/([^/]+)$/);
  if (caseMatch) {
    openRednoteCase(caseMatch[1], false);
    return;
  }

  const projectMatch = window.location.hash.match(/^#project\/([^/]+)$/);
  if (projectMatch) {
    if (!experienceDetailView.hidden && projectMatch[1] === "internships") {
      closeInternshipExperience(false);
    } else if (!caseDetailView.hidden && projectMatch[1] === "rednote") {
      closeRednoteCase(false);
    } else {
...
document.querySelector("#back-button").addEventListener("click", () => closeProject());
